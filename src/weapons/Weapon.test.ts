...
describe('Weapon system', () => {
  describe('createMeleeWeapon()', () => {
    it('creates a melee weapon from config', () => {
      const config = VEHICLE_STATS[VehicleType.Bicycle].weapons[WeaponSlot.Melee];
    ...
    it('Machete has high damage and fast speed', () => {
      const config = VEHICLE_STATS[VehicleType.Skateboard].weapons[WeaponSlot.Melee];
      const weapon = createMeleeWeapon(config);
  ...
  describe('createRangedWeapon()', () => {
    it('creates a ranged weapon from config with ammo', () => {
      const config = VEHICLE_STATS[VehicleType.Bicycle].weapons[WeaponSlot.Ranged];
    ...
    it('Crossbow has piercing enabled', () => {
      const config = VEHICLE_STATS[VehicleType.RollerBlades].weapons[WeaponSlot.Ranged];
      const weapon = createRangedWeapon(config);
    ...
    it('Shotgun has spread enabled', () => {
      const config = VEHICLE_STATS[VehicleType.Skateboard].weapons[WeaponSlot.Ranged];
      const weapon = createRangedWeapon(config);
  ...
  describe('combat scenario: melee vs ranged tradeoff', () => {
    it('melee never runs out but ranged eventually does', () => {
      const meleeConfig = VEHICLE_STATS[VehicleType.Bicycle].weapons[WeaponSlot.Melee];
      const rangedConfig = VEHICLE_STATS[VehicleType.Bicycle].weapons[WeaponSlot.Ranged];

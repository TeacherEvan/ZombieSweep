...
import type { NpcArchetypeSelection, NpcSpawnPlan } from './NpcScheduler';
import { buildNpcSpawnPlan, sanitizeSpawnZone, selectNpcArchetypes } from './NpcScheduler';
...
describe('NpcScheduler', () => {
  ...
  it('tilts toward hostile humans when blockade pressure spikes', () => {
    ...
    expect(['hostile-raider', 'hostile-paranoid-prepper']).toContain(plans[0].definition.id);
  ...
  it('skips invalid spawn zones safely', () => {
    ...
    expect(plans.every((plan: NpcSpawnPlan) => plan.spawnZone === 'FrontPorch')).toBe(true);
    expect(plans.some((plan: NpcSpawnPlan) => plan.spawnZone === invalidZone)).toBe(false);

...
import { createHole, createIcePatch, createLog } from '../entities/Hazard';
import type { House } from '../entities/House';
import { createColonialHouse, createRanchHouse, createVictorianHouse } from '../entities/House';
import { NpcTimeSlice } from '../entities/Npc';
import type { Pickup } from '../entities/Pickup';
import { PickupType, createPickup } from '../entities/Pickup';
import type { NpcScheduleContext, NpcSpawnPlan } from '../systems/NpcScheduler';
import { buildNpcSpawnPlan } from '../systems/NpcScheduler';
import { createTownReputation } from '../systems/TownReputation';
import type { DensityLevel, MapConfig } from './MapConfig';
...
const HOUSE_CREATORS = [createRanchHouse, createColonialHouse, createVictorianHouse];
...
export function generateRoute(
  ...
  const npcSpawns = generateNpcSpawns(mapConfig, difficulty, day, subscriberCount, hazards.length);
...
function generateHazards(mapConfig: MapConfig, difficulty: Difficulty): Hazard[] {
...
function generatePickups(): Pickup[] {
  ...
  const types = [PickupType.NewspaperBundle, PickupType.AmmoCrate, PickupType.HealthKit];
...
function generateNpcSpawns(
  ...
  const reputation = createTownReputation({
    ...
    alertness: THREAT_BASE_BY_DIFFICULTY[difficulty] + hazardCount * 2 + Math.max(0, day - 1) * 2,
  });
  ...
  const context: NpcScheduleContext = {
    ...
    routeTriggers: buildRouteTriggers(mapConfig, difficulty, subscriberCount, hazardCount),
    threatLevel: buildThreatLevel(difficulty, day, subscriberCount, hazardCount),
    reputation,
...
function buildThreatLevel(
  ...
  const subscriberPressure = Math.max(0, GAME.STARTING_SUBSCRIBERS - subscriberCount) * 4;
  const hazardPressure = hazardCount * 2;

  return Math.min(100, base + dayPressure + subscriberPressure + hazardPressure);
}
...
function buildNpcLimit(difficulty: Difficulty, subscriberCount: number): number {

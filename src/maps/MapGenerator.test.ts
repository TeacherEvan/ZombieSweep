...
describe('MapGenerator', () => {
  describe('generateRoute()', () => {
    it('generates exactly 20 houses', () => {
      const route = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1);
    ...
    it('generates hazards', () => {
      const route = generateRoute(MAPS[MapName.RustCreek], Difficulty.HardWay, 7);
      expect(route.hazards.length).toBeGreaterThan(0);
    });

    it('harder difficulty generates more hazards', () => {
      const easy = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1);
      const hard = generateRoute(MAPS[MapName.MapleGrove], Difficulty.HardWay, 1);
    ...
    it('subscriberCount param overrides default 10', () => {
      const route = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1, 5);
      const subscribers = route.houses.filter(h => h.isSubscriber);
  ...
  describe('route consistency', () => {
    it('same inputs produce same house count (deterministic structure)', () => {
      const r1 = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1);
      const r2 = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1);
    ...
    it('RustCreek Hard Way day 7 generates the most hazards', () => {
      const easy1 = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1);
      const hard7 = generateRoute(MAPS[MapName.RustCreek], Difficulty.HardWay, 7);
    ...
    it('fewer subscribers still produces 20 total houses', () => {
      const route = generateRoute(MAPS[MapName.MapleGrove], Difficulty.EasyStreet, 1, 3);
      expect(route.houses).toHaveLength(GAME.TOTAL_HOUSES);

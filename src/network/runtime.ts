...
import type { CoopRole, MultiplayerMode } from './protocol';
...
export function getCoopRuntimeState(registry: RegistryLike): CoopRuntimeState | null {
  const state = registry.get(COOP_RUNTIME_KEY);
  if (!state || typeof state !== 'object') return null;
  return state as CoopRuntimeState;
}

export function setCoopRuntimeState(registry: RegistryLike, state: CoopRuntimeState | null): void {
...
export function getCoopSession(registry: RegistryLike): MultiplayerSession | null {
  const session = registry.get(COOP_SESSION_KEY);
  return session instanceof MultiplayerSession ? session : null;
}

export function setCoopSession(registry: RegistryLike, session: MultiplayerSession | null): void {

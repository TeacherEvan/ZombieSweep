...
import { DeliveryDifficulty } from '../entities/House';
...
export function getVehicleControlProfile(vehicleType: VehicleType): VehicleControlProfile {
...
export function getZombieWaveSettings(
  ...
  const difficultyCountBonus =
    difficulty === Difficulty.HardWay ? 2 : difficulty === Difficulty.MiddleRoad ? 1 : 0;

  let count = Math.min(
    14,
    3 + Math.floor(dayGrowth * 1.1) + Math.floor(pressureDeliveries / 3) + difficultyCountBonus
  );
  count = Math.max(1, Math.round(count * adaptiveMultiplier));
  ...
  interval = Math.max(400, Math.round(interval / adaptiveMultiplier));
  ...
  const eliteChance = Math.min(
    ...
    0.06 +
      ...
      (difficulty === Difficulty.HardWay ? 0.12 : difficulty === Difficulty.MiddleRoad ? 0.06 : 0)
  );

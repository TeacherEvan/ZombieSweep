...
describe('arcade-rules', () => {
  describe('house texture keys', () => {
    it('maps subscriber and non-subscriber houses to the right texture key', () => {
      ...
      expect(getHouseTextureKey(createVictorianHouse(false))).toBe('victorian-nonsub');
    });
  });
  ...
  describe('vehicle control profile', () => {
    it('uses handling and stability to shape responsiveness', () => {
      ...
      expect(skateboard.inputResponsiveness).toBeGreaterThan(bicycle.inputResponsiveness);
      expect(bicycle.coastResponsiveness).toBeGreaterThan(skateboard.coastResponsiveness);

...
describe('vehicle config', () => {
  ...
  it('Crossbow is the only piercing weapon', () => {
    const crossbow = VEHICLE_STATS[VehicleType.RollerBlades].weapons[WeaponSlot.Ranged];
    const slingshot = VEHICLE_STATS[VehicleType.Bicycle].weapons[WeaponSlot.Ranged];
    const shotgun = VEHICLE_STATS[VehicleType.Skateboard].weapons[WeaponSlot.Ranged];

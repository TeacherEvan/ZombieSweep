...
describe('game constants', () => {
  ...
  describe('NEWSPAPER', () => {
    ...
    it('Sunday paper speed = base × 0.6', () => {
      const sundaySpeed = NEWSPAPER.BASE_THROW_SPEED * NEWSPAPER.SUNDAY_SPEED_MULTIPLIER;
  ...
  describe('CITIZEN_PENALTY', () => {
    ...
    it('panicked runner has the harshest penalty', () => {
      expect(CITIZEN_PENALTY.PANICKED_RUNNER).toBeLessThan(CITIZEN_PENALTY.FRIENDLY_NEIGHBOR);
      expect(CITIZEN_PENALTY.PANICKED_RUNNER).toBeLessThan(CITIZEN_PENALTY.ARMED_SURVIVALIST);
    });
  });
